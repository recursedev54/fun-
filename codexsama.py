...
def generate_response(user_input, ngram, window_size):
    ...
    while len(out.split()) < len(user_input.split()):
        if chosen_word_pair not in ngram.keys():
            break
        next_word = random.choice(ngram[chosen_word_pair])
        out += ' ' + next_word
...
#to make ur own corpus just copy paste whatever u want into corpus
#like this corpus = """ur words go here. another sentence. blah blah blah."""
