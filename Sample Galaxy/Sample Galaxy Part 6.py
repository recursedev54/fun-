...
class SampleGalaxyApp:
    def __init__(self, root):
        ...
        self.angle_offset = 0  # Initial angle offset for the spiral arms
    ...
    def draw_spiral_galaxy(self, angle_offset):
        ...
        arm_length = 100  # Length of each arm
        ...
        for arm in range(num_arms):
            for i in range(num_stars_per_arm):
                ...
                distance_from_center = arm_length * (1 + i / num_stars_per_arm)

                # Calculate the position of the star
                x = center_x + distance_from_center * math.cos(angle + arm * 2 * math.pi / num_arms + angle_offset)  # Add offset for each arm
                y = center_y + distance_from_center * math.sin(angle + arm * 2 * math.pi / num_arms + angle_offset)
