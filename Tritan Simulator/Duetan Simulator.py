...
def duetan_simulation(hex_color):
    ...
    if not all(c.isdigit() or c.lower() in 'abcdef' for c in hex_color1[1:]) or len(hex_color1) != 7:
        print("Invalid hex generated:", hex_color1)
    if not all(c.isdigit() or c.lower() in 'abcdef' for c in hex_color2[1:]) or len(hex_color2) != 7:
        print("Invalid hex generated:", hex_color2)
