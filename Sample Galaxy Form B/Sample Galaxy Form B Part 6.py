...
def update_window_position(event):
    ...
    # Position the mask window relative to the main canvas
    mask_window.geometry(f"800x600+{root_x}+{root_y}")
class ClippingMask:
    ...
    def create_oval_with_alpha(self, x1, y1, x2, y2, fill):
        r, g, b, a = hex_to_rgb_and_alpha(fill)
        color = f'#{r:02x}{g:02x}{b:02x}'

        # Create the oval with background color
        custom_oval = self.canvas.create_oval(x1, y1, x2, y2, fill=color, outline="")

        # Apply transparency using an overlay canvas
        overlay_canvas = tk.Canvas(self.canvas, width=x2 - x1, height=y2 - y1, highlightthickness=0)
        overlay_canvas.create_oval(0, 0, x2 - x1, y2 - y1, fill="black", outline="")
        overlay_canvas.place(x=x1, y=y1)

        # Set the transparency level
        overlay_canvas.configure(bg=overlay_canvas.cget('bg'), highlightthickness=0, bd=0)
        overlay_canvas.create_oval(0, 0, x2 - x1, y2 - y1, fill="white", outline="")
        overlay_canvas.lift(custom_oval)

        return custom_oval
    
def track_items_inside_clipping_mask(self, clipping_mask):
    items_inside = []
    items_passing_through = []

    # Get the coordinates of the clipping mask
    x1, y1, x2, y2 = self.canvas.coords(clipping_mask)

    # Debug: Print the coordinates of the clipping mask
    print("Clipping Mask Coordinates:", x1, y1, x2, y2)

    # Iterate through all items on the canvas
    for item in self.canvas.find_all():
        # Get the coordinates of the current item
        item_x1, item_y1, item_x2, item_y2 = self.canvas.coords(item)

        # Debug: Print the coordinates of the current item
        print("Item Coordinates:", item_x1, item_y1, item_x2, item_y2)

        # Check if the item's coordinates intersect with the clipping mask's coordinates
        if (item_x1 >= x1 and item_y1 >= y1 and item_x2 <= x2 and item_y2 <= y2):
            items_inside.append(item)
        elif (item_x1 <= x2 and item_y1 <= y2 and item_x2 >= x1 and item_y2 >= y1):
            items_passing_through.append(item)

    if items_passing_through:
        print("Items passing through the clipping mask:", items_passing_through)

    return items_inside


    def create_mask(self):
        self.mask = self.create_oval_with_alpha(300, 220, 500, 420, fill="#FFFFFF80")  # Adjust coordinates for the circular mask
...
if __name__ == "__main__":
    ...
    window2.geometry("800x600")  # Set the size of window2

    canvas2 = tk.Canvas(window2, width=800, height=600, bg="black")
    canvas2.pack()

    # Create clipping mask window
    mask_window = tk.Toplevel(root)
    mask_window.overrideredirect(True)
    mask_window.attributes("-transparentcolor", "black")
    mask_window.geometry("800x600")  # Set the size of the mask window

    # Initialize clipping mask
    clipping_mask = ClippingMask(mask_window)

    # Call the function to track items inside the clipping mask
    clipping_mask.track_items_inside_clipping_mask(clipping_mask.mask)
